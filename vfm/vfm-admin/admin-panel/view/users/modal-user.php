                ...
                <form role="form" method="post" autocomplete="off" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>?section=users&action=update" enctype="multipart/form-data" class="removegroup">
                    ...
                    <div class="row">
                        ...
                        if (strlen($setUp->getConfig('email_from')) > 4) { ?>
                            <div class="usernotif_again">
                                <button type="button" class="btn btn-link notify_again"><i class="bi bi-envelope"></i> <?php echo $setUp->getString("send_password_reset"); ?></button>
                                <input type="hidden" name="usernotif_again" id="usernotif_again">
                            </div>
                            <?php
                        } ?>
                        </div>
